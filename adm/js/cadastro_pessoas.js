  $(document).ready(function(){
        		...
        		$("#chkFiliado").change(function(){
        			var parent = $(this).parents('.switch');
        			if($("#chkFiliado").attr('checked')==true){
        		...
        		$("#chkMovimento").change(function(){
        			var parent = $(this).parents('.switch');
        			if($("#chkMovimento").attr('checked')==true){
        				$(".cb-enable",parent).click();
        			}else{
        				$(".cb-disable",parent).click();
        			}
...
function iniciar(){
	...
	comboDataNasc();
	esconder("chkFiliado");
	esconder("chkMovimento");
	carregaComboPartido();
	
	var varXML = chamar_ajax('php/sql.php', 'filtro=CarregaFiliado', false, 'xml', null);
...
function esconder(checkName){
	if(checkName=="chkFiliado"){
		if($("#chkFiliado").attr('checked')==true){
			$("#cboPartido").show(200);
		}else{
			$("#cboPartido").hide('fast'); cboPartido
			document.getElementById("cboPartido").value = "";
		}
	}
	if(checkName=="chkMovimento"){
		if($("#chkMovimento").attr('checked')==true){
			$("#lblQual").show(200);
		}else{
			$("#lblQual").hide('fast');
...
function carregaComboPartido() {

	var varXML = chamar_ajax('php/sql.php', 'filtro=CarregaComboPartido', false, 'xml', null);
	carregar_combo(varXML, 'cboPartido', 'sigla_partido', 'id_partido'); 
...
function submit() {
	...
	//Validação OK - Inserindo no banco
	if (validar()) {
		frmPessoa.submit();
